...
class _EditPostScreenState extends State<EditPostScreen> {
  ...
  Future<Post> getRequest() async {
    final String url =
        "http://localhost:5000/postInfo/getPostById/" + widget.postId;
  ...
  Future<Post> deletePost() async
  {
    final http.Response response = await http.delete(
        Uri.parse("http://localhost:5000/postInfo/deletePostById/"+widget.postId),
  ...
  Future<Post> updatePost(String title, String description , String imageUrl) async {
    final http.Response response = await http.patch(
      Uri.parse("http://localhost:5000/postInfo/updatePostById/"+widget.postId),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, dynamic>{
