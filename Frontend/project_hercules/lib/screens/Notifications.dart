...
class NotificationScreen extends StatelessWidget {
  ...
  Future<HashMap<Message, User>> searchMessage() async {
    ...
    final response = await http
        .get(Uri.parse("http://localhost:5000/chatInfo/search/" + userId));
    ...
    for (var eachMessage in responseData) {
      ...
      final response2 = await http.get(Uri.parse(
          "http://localhost:5000/personalInfo/getUserById/" +
              message.fromId));
  ...
  Future<User> getUser() async {
    final String url =
        "http://localhost:5000/personalInfo/getUserById/" + userId;
