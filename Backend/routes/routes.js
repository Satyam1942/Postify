...
router.patch('/updateUserPasswordById/:id', async(req, res) => {
    try{
     const id = req.params.id;
     const updatedData =  generateHash(req.body.password);
     const options = {new : true};
     const finalPassword = {"password":updatedData}
    const result = await model.findByIdAndUpdate(id,finalPassword,options)
    res.status(200).send(result);
    }catch (error) {
         res.status(400).json({message: error.message})
     }
